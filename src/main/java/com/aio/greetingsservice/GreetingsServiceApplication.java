package com.aio.greetingsservice;
...
class RocketSecurityConfiguration {
...
class GreetingsService {
    ...
    Flux<GreetingsResponse> greet(@AuthenticationPrincipal Mono<UserDetails> user) {
        return user
                .map(UserDetails::getUsername)
                .flatMapMany(GreetingsService::greet);
    }

    private static Flux<GreetingsResponse> greet(String name) {
        return Flux.fromStream(
                Stream.generate(new Supplier<GreetingsResponse>() {
                    @Override
                    public GreetingsResponse get() {
                        return new GreetingsResponse("Hello " + name + " @ " + Instant.now().toString());
                    }
                })
